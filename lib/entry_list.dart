...
class EntryList extends StatelessWidget {
  ...
  EntryList(this.entries, {required this.deleteEntry});

  /// called for every UI list entry being built
  Widget _buildEntry(BuildContext context, int index) {
    return GestureDetector(
      onTap: () => Navigator.pushNamed<bool>(context, '/book/$index')
  ...
  Widget _buildEntryList() {
    return entries.length > 0
        ...
        : Center(
            child: Text(
                'Your bookself is empty, for adding please click menu icon!'),
          );
  }
